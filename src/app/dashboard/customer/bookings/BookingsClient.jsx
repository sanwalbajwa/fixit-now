...
import { cancelMyBooking, submitBookingRating } from '@/lib/actions/bookings'
...
export default function BookingsClient({ bookings }) {
  ...
  if (filter === 'pending') {
    filteredBookings = visibleBookings.filter((b) => ['pending', 'confirmed', 'accepted'].includes(b.status))
  } else if (filter === 'active') {
    filteredBookings = visibleBookings.filter((b) => !['completed', 'cancelled'].includes(b.status))
        ...
        <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed border-slate-200 bg-white py-20 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              {filter === 'pending' && 'Bookings awaiting confirmation will appear here.'}
              {filter === 'active' && 'In-progress bookings will appear here.'}
